...
pub struct Scheduler {
    heap: BinaryHeap<Reverse<(u32, Vec<Vec<[usize; 3]>>)>>, // Min-heap of (time, updates)
}
...
impl Scheduler {
    ...
    pub fn push(&mut self, coords: Vec<Vec<[usize; 3]>>, time: f64) {
    ...
    pub fn push_all(&mut self, updates: Vec<(f64, Vec<Vec<[usize; 3]>>)>) {
